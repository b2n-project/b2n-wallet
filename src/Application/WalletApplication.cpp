...
const char BYTECOIN_URI_SCHEME_NAME[] = "bitcoin2network";
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("b2nwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] bitcoin2network wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "bitcoin2network wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have bitcoin2networkd with non-default RPC port, you should terminate it and relaunch B2Nwallet\n"
      "or\n"
      "Set the Local deamon required port in B2Nwallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
