...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("b2nwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("b2nwallet.desktop");
  ...
  res = autorunSettings.contains("B2Nwallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("B2Nwallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.bitcoin2network.bitcoin2network scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=bitcoin2network Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("B2Nwallet", appPath);
    } else {
      autorunSettings.remove("B2Nwallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoin2network", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:bitcoin2network");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoin2network\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\bitcoin2network\\shell\\open\\command", QSettings::NativeFormat);
