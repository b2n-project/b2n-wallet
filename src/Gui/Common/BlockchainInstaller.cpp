...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_bitcoin2networkDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfbitcoin2networkBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 bitcoin2networkHeight;
  if (!checkIfBlockchainOutdated(currentHeight, bitcoin2networkHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(bitcoin2networkHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfbitcoin2networkBlockchainExists() const {
  return m_bitcoin2networkDir.exists() && m_bitcoin2networkDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_bitcoin2networkDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::bitcoin2networkBlockchainInfo() const {
  return QFileInfo(m_bitcoin2networkDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_bitcoin2networkDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_bitcoin2networkDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
